...
using VisProcess.ViewModel;

namespace VisProcess.Services
{
    public class AccountService
    {
        private readonly TableWork _tableWork = new TableWork();

        public LoginViewModel ExternalLogin(Usertemporary user)
        {
        ...
        {
            VisTestContext db = new VisTestContext();
            var Emp = (from E in db.Employees
                       join D in db.Designations on E.DesignationId equals D.Id
                       join DE in db.Departments on E.DepartmentId equals DE.Id
        ...
        {
            VisTestContext db = new VisTestContext();
            var KraDes = (from E in db.Employees
                          join KRA in db.KravsDesignations on E.DepartmentId equals KRA.DepartmentId
        ...
        public Object CreateKRAApproval(KRA para)
        {
            ...
            try
            {
                ...
                var KraDes = db.KravsDesignations.Where(x => x.DepartmentId == Emp.DepartmentId && x.KraisDeleted == false).FirstOrDefault();
                if (KraDes != null)
                {
                    var K = db.Kraapprovals.Where(x => x.EmployeeId == para.EmployeeId).FirstOrDefault();
                    if (K == null)
                    {
        ...
        public Object CreateWellnessStatus(WELL para)
        {
            ...
            try
            {
                ...
                if (Emp != null)
                {
                    ...
                    var Empcheck = db.WellnessStatuses.Where(x => x.EmployeeId == para.EmployeeId).FirstOrDefault();
                    if (Empcheck == null)
                    {
                        //var Monthcheck = db.WellnessStatus.Where(x => x.Monthlystatus != DateTime.Today.Month).FirstOrDefault();-----old
                        var Monthcheck = db.WellnessStatuses.Where(x => x.Monthlystatus == DateTime.Today.Month && x.EmployeeId == para.EmployeeId).FirstOrDefault();//-----new
                        if (Monthcheck == null)
                        {
        ...
        public Object GetEmployeeProcess(int EmployeeId)
        {
            ...
            using (var dataContext = new VisTestContext())
            {
                var employeProcess = (from EP in dataContext.EmployeeProcesses
                                      join P in dataContext.Processes on EP.ProcessId equals P.Id
